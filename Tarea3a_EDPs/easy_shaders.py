# coding=utf-8
"""
Daniel Calderon and Sebastián Salinas, CC3501, 2020-1
Simple Shaders
"""
...
def textureSimpleSetup(texture, imgName, wrapMode, filterMode):
    ...
    image = Image.open(imgName)  #Aqui la libreria pillow de python va a buscar la imagen al disco duro y la guarda en memoria RAM
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, image.size[0], image.size[1], 0, format, GL_UNSIGNED_BYTE, img_data) # Aqui se envia a la tarjeta de video (GPU)
#Para que OpengGL lo pueda utlizar 



def toGPUShape(shape, wrapMode=None, filterMode=None): # esta funcion había que modificar para la decima doble
    assert isinstance(shape, bs.Shape)                 # que reciba la textura ya guardada y no la cargue siempre
...
class SimpleShaderProgram:
    ...
    def drawShape(self, shape, mode=GL_TRIANGLES): #El 3er parametro es lo que dibujara por defecto es triangulo
        assert isinstance(shape, GPUShape) #pero podria ser lineas, puntos, etc
...
class SimpleModelViewProjectionShaderProgram_Transparency:
    ...
    def __init__(self):
        ...
        fragment_shader = """
            ...
            void main()
            {
                outColor = vec4(newColor, 0.1f); 
            }
            """#0.1 muy transparente uwu
...
class SimpleModelViewProjectionShaderProgram_ExtremeTransparency:
